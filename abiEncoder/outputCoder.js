...
outputCoder.getProofOutput = (proofOutputsHex, i) => {
    const proofOutputs = proofOutputsHex.slice(2);
    const offset = (i * 0x40) + 0x40;
    const location = parseInt(proofOutputs.slice(offset, offset + 0x40), 16);
    const length = parseInt(proofOutputs.slice(location * 2 - 0x40, location * 2), 16);
    return proofOutputs.slice(location * 2 - 0x40, (location * 2) + (length * 2));
};
