...
/// @dev libraries cannot have their own storage variables (they have no persistant state)
/// @dev They are designed to have their methods called by other contracts or libraries
/// @dev There are two fundamental ways smart contracts can interface with a library
/// @dev The first is by referencing a 'library' smart contract instantiated at a specific address.
/// @dev The second is to incorporate a library's bytecode directly into a smart contract (done by the compiler)
/// @dev The former is useful for complex methods as it means multiple smart contracts can reference a single
/// @dev library and reduce the amount of raw bytecode that needs to be deployed on-chain
/// @dev The latter is useful for small helper functions, where the gas cost of calling an external smart contract
/// @dev would be excessive. This is done by declaring a library function to be `internal`. This notifies the
/// @dev Solidity compiler to incorporate the function's bytecode in any contract that uses the library.

/// @notice It costs 700 gas to call an external library instantiated at a specific smart contract address.
/// @notice When creating a smart contract, it costs 200 gas per byte of bytecode the contract contains.
/// @notice Whether a library's methods should be internal or not depends on which of the above two
/// @notice is worth minimizing for a given method.
