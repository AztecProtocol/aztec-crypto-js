...
helpers.toBnAndAppendPoints = (proofData) => {
    const proofDataBn = proofData.map((noteData) => {
        // Reconstruct gamma
        const xGamma = new BN(noteData[2].slice(2), 16).toRed(bn128.curve.red);
        const yGamma = new BN(noteData[3].slice(2), 16).toRed(bn128.curve.red);
        const gamma = bn128.curve.point(xGamma, yGamma);

        // Reconstruct sigma
        const xSigma = new BN(noteData[4].slice(2), 16).toRed(bn128.curve.red);
        const ySigma = new BN(noteData[5].slice(2), 16).toRed(bn128.curve.red);
